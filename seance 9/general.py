...
from niveau1 import get_plateforme_prison, get_plateformes, get_plateforme_danger, get_sol, get_mur
import sfx
# ----------------------------
...
dialogue_g = False
cadre_g = pygame.image.load("images/cadre_dialogue.png").convert_alpha()
...
plateformes = get_sol() + plateformes_prison + plateformes_niveau
niveau_largeur = 2000  # largeur totale du niveau
try:
    plateforme_img = pygame.image.load("images/plateforme_moyenne.png").convert_alpha()
    sol_img = pygame.image.load("images/sol.png").convert_alpha()
except:
    plateforme_img = None
    sol_img = None
...
while running:
    ...
    for plat in plateformes:
        # Choisir l'image à appliquer
        if plat == sol and sol_img:
            img = sol_img
        elif plateforme_img:
            img = plateforme_img
        else:
        ...
        if img:
            # Redimensionner l'image à la taille du rectangle
    ...
    # Affichage du mur
    for plat in mur:
        pygame.draw.rect(screen,(255, 0, 0),(plat.x - camera_x,plat.y - camera_y,plat.width,plat.height))
