...
from niveau1 import get_plateforme_prison, get_plateformes, get_plateforme_danger, get_sol, get_plateformeshaute
import sfx
# ----------------------------
...
inventaire = pygame.transform.scale(pygame.image.load("images/inventaire.png").convert_alpha(), (560, 630))
...
messages = ["GALILEO !!",
            "C'EST UN ENFER !",]
#npc 2
virgilio = pygame.image.load("images/virgilio.png").convert_alpha()
virgilio = pygame.transform.scale(virgilio, (56, 112))
...
speed =  5
active_message2 = 0
message = messages[active_message2]
done = False
message_v = ["Prends cette potion pour sauter deux fois."]
active_message2 = 0
message2 = message_v[active_message2]

giordano_cooldown = -30000 #commence à -30 secondes pour que le joueur puisse gagner une vie dès le début du jeu s'il parle au npc
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_e and dialogue_g and done:
                if active_message < len(messages) - 1:
                    ...
                    #le joueur gagne une vie si il parle au npc mais il y a un délai d attente pour pas que le joueur puisse gagner pleins 
                    #de vies 
                ...
                active_message2 = 0
                message = message_v[0]
            elif event.key == pygame.K_e and pancarte_active and lire_pancarte == False:
                lire_pancarte = True
                joueur.peut_bouger = False
    ...
    joueur.appliquer_gravite(plateformes + sol, murs = plateformes_haute)
    ...
    # Affichage du panneau et interaction avec le panneau
    ...
    #Affichage des plateformes hautes
    for plat_haute in plateformes_haute:
        pygame.draw.rect(screen,(0,0,0),(plat_haute.x - camera_x, plat_haute.y - camera_y, plat_haute.width,plat_haute.height))
    ...
    # dialogue quand le joueur est proche de virgilio
    ...
    if inventaire_affiche:
        inv_x = screen_width // 2 - inventaire.get_width() // 2
        inv_y = screen_height // 2 - inventaire.get_height() // 2
        screen.blit(inventaire, (inv_x, inv_y))
    if show_button_e_inventaire:
        screen.blit(bouton_e, (850, 600 + button_offset))

    # Si on lit, on affiche la pancarte
    if lire_pancarte == True:
