...
class Joueur(pygame.sprite.Sprite):
    ...
    def deplacement(self, plateformes):
        ...
        if touches[pygame.K_a] and self.couteau_equipee and not self.is_attacking:
            if temps_actuel - self.cooldown_attaque >= 2500:  # réutilise self.cooldown_attaque comme cooldown attaque
                self.is_attacking = True
                self.cooldown_attaque = temps_actuel
                couteausfx.play()

        if self.is_attacking:
            if temps_actuel - self.cooldown_attaque >= 1000:
                self.is_attacking = False
            else:
                if self.facing_left:
