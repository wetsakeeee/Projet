...
from niveau1 import get_plateforme_prison, get_plateformes, plateforme_pic, plateforme_pic2, get_sol, get_plateformeshaute, get_sol2, mur2, plateforme_2, bateau
import sfx
...
epee                       = pygame.transform.scale(pygame.image.load("images/Objets/couteau.png").convert_alpha(), (80, 80))
...
bateau_caronte = pygame.transform.scale(pygame.image.load("images/Divers/bateau.png").convert_alpha(), (300,130))
bateau_rect = bateau_caronte.get_rect()
...
def charger_image_optionnelle(path):
    try:
        return pygame.image.load(path).convert_alpha()
    except Exception:
        return None

platform_image_orig = charger_image_optionnelle("images/Plateformes/Niveau1/plateforme_moyenne.png")
platform_petite_orig = charger_image_optionnelle("images/Plateformes/Niveau1/plateforme_petite.png")
plateforme2_img = charger_image_optionnelle("images/Plateformes/Niveau2/plateforme2.png")
mur_prison_orig = charger_image_optionnelle("images/Plateformes/Niveau1/murprison.png")
sol_image_orig = charger_image_optionnelle("images/Plateformes/Niveau1/sol.png")
pic_sol_orig = charger_image_optionnelle("images/Plateformes/pic_sol.png")
pic_plafond_orig = charger_image_optionnelle("images/Plateformes/pic_plafond.png")
sol2_image_orig = charger_image_optionnelle("images/Plateformes/Niveau2/sol_niveau2.png")
mur2_image_orig = charger_image_optionnelle("images/Plateformes/Niveau2/mur_niveau2.png")
mur2_2_image_orig = charger_image_optionnelle("images/Plateformes/Niveau2/mur2_niveau2.png")
...
def reset():
    ...
    global dialogue_g, dialogue_v, dialogue_c1, dialogue_caronte
...
niveau_actuel = 1 # si 2, niveau caronte, si 3, boss satan
# -------------------------------------------------------------------------------------------------#
# Niveau 2 avec Caronte
# -------------------------------------------------------------------------------------------------#
fond_caronte = pygame.transform.scale(pygame.image.load("images/Fonds/fond_caronte.png").convert_alpha(), (2560, 1440))
bateau = bateau()
bateau_img = pygame.transform.scale(pygame.image.load("images/Divers/bateau.png").convert_alpha(), (300,130))

bateau_image = []
for b in bateau:
    img = pygame.transform.scale(bateau_img, (b.width, b.height))
...
while running:
    ...
    if niveau_actuel == 1:
        ...
        active = active2 = active3 = active4 = False
                ...
                #---Caronte
                    ...
                    if not done:
                        ...
                        if active_message_caronte < len(message_caronte) - 1:
                            ...
                            niveau_actuel = 2
                            joueur.rect.centerx = bateau[0].centerx
                            joueur.rect.bottom = bateau[0].top
                            joueur.vel_y = 0
                            joueur.vx = 0
    ...
    if not en_pause and not transition_recommencer and not transition_porte_enfer_start and etat != "mort":
        if niveau_actuel == 1:
            ...
            joueur.deplacement(bateau + plateformes_haute)
            joueur.appliquer_gravite(bateau + plateformes_haute, murs=plateformes_haute)
        if not invincible:
    ...
    if niveau_actuel == 1:
        ...
        screen.blit(fond_caronte, (-camera_x, -camera_y))
...
# Parcours deux listes avec zip, celles des plateformes et des images correspondantes, pour afficher chaque plateforme à sa position avec la bonne image.
    ...
    for plat, img in zip(bateau, bateau_image):
        if img:
            screen.blit(img, (plat.x - camera_x, plat.y - camera_y))
