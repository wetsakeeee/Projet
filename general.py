...
def appliquer_parametre_jeu(param_name, enabled):
    """applique les changements dans les paramètres dans le jeu"""
...
def initialiser_jeu():
    ...
    global bouton_go_rejouer_img, bouton_go_menu_img, bouton_go_quitter_img
    ...
    global BULLE_DUREE, BULLES_TEXTES_MONSTRE, BULLES_TEXTES_ESPRIT, BULLES_TEXTES_SQUELETTE
    ...
    bouton_go_rejouer_img = pygame.transform.scale(pygame.image.load("images/GUI/Menu/rejouer.png").convert_alpha(), (480, 140))
    bouton_go_menu_img   = pygame.transform.scale(pygame.image.load("images/GUI/Menu/menu_debut.png").convert_alpha(), (480, 140))
    bouton_go_quitter_img = pygame.transform.scale(pygame.image.load("images/GUI/Menu/quitter.png").convert_alpha(), (480, 140))

    bouton_rejouer = bouton_go_rejouer_img.get_rect(center=(screen_width//2, 430))
    bouton_menu_go = bouton_go_menu_img.get_rect(center=(screen_width//2, 520))
    bouton_quitter = bouton_go_quitter_img.get_rect(center=(screen_width//2, 590))
    background_game_over = pygame.transform.scale(pygame.image.load("images/Fonds/image3.jpg").convert(), (screen_width, screen_height))
    gameover_img = pygame.transform.scale(pygame.image.load("images/Fonds/gameover.png").convert_alpha(), (400, 400))
...
def get_slot_inventaire_rect(index):
    """crée un rectangle pour un slot d inventaire à partir de l index"""
...
def trouver_index_inventaire(item_id):
    """trouve l index d un objet dans l inventaire"""
...
def ajouter_vie():
    """ajoute une vie au joueur, jusqu'à un maximum de 3"""
    global vies
    if vies < 3:
        vies += 1
        sfx.viesfx.play()
...
def dessiner_texte_contour(surface, font, texte, x, y, couleur_texte, couleur_contour, epaisseur=3):
    """affiche du texte avec un contour"""
...
def synchroniser_bottes_double_saut():
    """synchronise les bottes de double saut entre le joueur et l inventaire."""
...
def synchroniser_couteau():
    """synchronise le couteau entre le joueur et l inventaire."""
...
def synchroniser_cles_finales():
    """synchronise les clés de Caronte et de l Enfer entre le joueur et l inventaire."""
...
def mettre_speedrun_en_pause(temps_actuel):
    """met le timer de speedrun en pause"""
...
def reprendre_speedrun_apres_pause(temps_actuel):
    """reprend le timer de speedrun après une pause"""
...
def couper_sons_pour_mort():
    ...
    for son in (sfx.musiquefond, sfx.degat, sfx.viesfx, sfx.sfxtitre, sfx.fin,
                ...
                sfxmarche1, sfxmarche2, sfxmarche3, sauter, tombersfx, sfx.niveau_2caronte):
        son.stop()

def reset():
    """remet le jeu à son état initial """
    global etat_global
...
while running:
    ...
    if etat == "game_over":
        ...
        screen.blit(gameover_img, gameover_img.get_rect(center=(screen_width//2, 225)))
        screen.blit(bouton_go_rejouer_img, bouton_rejouer)
        screen.blit(bouton_go_menu_img, bouton_menu_go)
        screen.blit(bouton_go_quitter_img, bouton_quitter)
        screen.blit(curseur_img, mouse_pos)
        pygame.display.flip()
        continue
    ...
    if joueur.couteau_equipee and not en_pause:
        ...
        if temps_ecoule < 2500:
            ...
            screen.blit(cooldown_imgs[frame_cd], (joueur.rect.x - camera_x, joueur.rect.y - camera_y - 70))
